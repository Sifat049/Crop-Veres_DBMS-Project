...
async function loadAllUsers(){
  ...
  try{
    ...
    usersBox.innerHTML = items.slice(0,60).map(u => {
      const canRemove = (u.role === 'buyer' || u.role === 'farmer');
      return `
      <div class="userRow fade">
        <div>
          <b>${u.name}</b> <span class="pill">${u.role}</span>
        ...
        <div class="actions">
          ${canRemove ? `<button class="danger" onclick="removeUser(${u.user_id}, '${u.name}')">Remove</button>` : ''}
        </div>
      </div>
    `;
    }).join("");
...
window.removeUser = async function(user_id, name){
  if (!confirm(`Remove ${name}? This will delete related data.`)) return;
  usersMsg.textContent = "";
  try {
    const r = await apiFetch(`/api/admin/users/${user_id}`, { method: "DELETE" });
    usersMsg.textContent = r.message || "User removed.";
    await loadAllUsers();
    await loadPending();
  } catch (e) {
    usersMsg.textContent = e.message || "Remove failed.";
  }
}
